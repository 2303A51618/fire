    ...
    <div className="space-y-8">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <p className="text-gray-600 text-sm">Fire Detected</p>
            <p className="text-3xl font-bold text-fire-600">{stats.fire_predictions || 0}</p>
          </Card>
          <Card>
            <p className="text-gray-600 text-sm">No Fire</p>
            <p className="text-3xl font-bold text-green-600">{stats.no_fire_predictions || 0}</p>
          </Card>
          <Card>
            <p className="text-gray-600 text-sm">Active Alerts</p>
            <p className="text-3xl font-bold text-orange-600">{stats.total_alerts || 0}</p>
          </Card>
        </div>
      )}
                ...
                <tr key={idx} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold">{(alert.confidence * 100).toFixed(2)}%</span>
                    </div>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3">{alert.email_sent ? '✅ Yes' : '❌ No'}</td>
                </tr>
              ))}
            </tbody>
          </table>
